...
public class DragItemRecyclerView extends RecyclerView implements com.emika.app.features.draglistview.AutoScroller.AutoScrollListener {
    private static final String TAG = "DragItemRecyclerView";
...
//            Log.d(TAG, "onDragging() called with: x = [" + x + "], y = [" + y + "]");
        }
        invalidate();
    }
    ...
    void onDragEnded() {
        ...
        if (mDisableReorderWhenDragging) {
            ...
            if (newPos != NO_POSITION) {
                ...
                Log.d(TAG, "onDragEnded() called" + newPos);
                mDragItemPosition = newPos;
            }
    ...
    void addDragItemAndStart(float y, Pair item, long itemId) {
    ...
    Pair removeDragItemAndEnd() {
        if (mDragItemPosition == NO_POSITION) {
            return null;
        }
        mAutoScroller.stopAutoScroll();
        Pair item = mAdapter.removeItem(mDragItemPosition);
        mAdapter.setDragItemId(NO_ID);
        mDragState = DragState.DRAG_ENDED;
        mDragItemId = NO_ID;
